<template>
  <div class="layout-tabs">
    <IconFold v-if="layout === 'common'" />
    <el-tabs
      v-model="tabActive"
      class="layout-tabs-content"
      :class="{
        ['layout-tabs-content-' + theme.tabsBarStyle]: true,
      }"
      type="card"
    ...
    >
      <el-tab-pane v-for="item in visitedRoutes" :key="item.path" :closable="!isNoCLosable(item)" :name="item.path">
        <template #label>
          <span style="display: inline-block" @contextmenu.prevent="openMenu($event, item)">
            <template v-if="theme.showTabsBarIcon">
              <!-- TODO 实现 :is-custom-svg="item.meta.isCustomSvg" -->
              <AppIcon v-if="item.meta && item.meta.icon" :icon="item.meta.icon" />
              <!--  如果没有图标那么取第二级的图标 -->
              <AppIcon v-else :icon="item.parentIcon" />
            </template>
            <span>
    ...
    <el-dropdown @command="handleCommand" @visible-change="handleVisibleChange">
      <span class="layout-tabs-more" :class="{ 'layout-tabs-more-active': active }">
        <span class="layout-tabs-more-icon">
          <i class="box box-t"></i>
          <i class="box box-b"></i>
        </span>
      </span>
      <template #dropdown>
        <el-dropdown-menu class="tabs-more">
          <el-dropdown-item command="closeOthersTabs">
            <AppIcon icon="el-icon-minus" />
            {{ translateTitle('关闭其他') }}
          </el-dropdown-item>
          <el-dropdown-item command="closeLeftTabs">
            <AppIcon icon="el-icon-back" />
            {{ translateTitle('关闭左侧') }}
          </el-dropdown-item>
          <el-dropdown-item command="closeRightTabs">
            <AppIcon icon="el-icon-right" />
            {{ translateTitle('关闭右侧') }}
          </el-dropdown-item>
          <el-dropdown-item command="closeAllTabs">
            <AppIcon icon="el-icon-close" />
            {{ translateTitle('关闭全部') }}
          </el-dropdown-item>
        </el-dropdown-menu>
      </template>
...
<script>
  ...
  export default {
    ...
    setup() {
      ...
      const addVisitedRoute = (_route) => store.dispatch('tabs/addVisitedRoute', _route)
      const delVisitedRoute = (rawPath) => store.dispatch('tabs/delVisitedRoute', rawPath)
      const delOthersVisitedRoutes = (activePath) => store.dispatch('tabs/delOthersVisitedRoutes', activePath)
      const delLeftVisitedRoutes = (activePath) => store.dispatch('tabs/delLeftVisitedRoutes', activePath)
      const delRightVisitedRoutes = (activePath) => store.dispatch('tabs/delRightVisitedRoutes', activePath)
      const delAllVisitedRoutes = (activePath) => store.dispatch('tabs/delAllVisitedRoutes', activePath)
      ...
      const addTabs = async (tag, init = false) => {
        ...
        if (tag.name && tag.meta.title && tag.meta.tabHidden !== true) {
          ...
          await addVisitedRoute({
            ...
            params: tag.params,
            name: tag.name,
            matched: init ? [tag.name] : tag.matched.map((_route) => _route.name),
            parentIcon,
            meta: { ...tag.meta },
          })
        ...
        :deep() {
          ...
          .el-tabs__header {
            ...
            .el-tabs__item {
              ...
              margin-top: #{math.div($base-tabs-height - $base-tag-item-height - 4, 2)};
              margin-right: -18px;
              line-height: $base-tag-item-height + 4;
              text-align: center;
              border: 0;
              ...
              &.is-active {
                ...
                background: mix($base-color-white, $base-color-blue, 90%);
                outline: none;
                mask: url('~@/assets/images/tabs_images/layout-tab.png');
                mask-size: 100% 100%;
